...
export class MemoryArchiver {
     ...
     */
    constructor(workspace, retentionDays = 30) {
    ...
    async cleanupOldArchives(days) {
        ...
        console.log(`[MemoryArchiver] Cleanup complete: ${deletedCount} files, ` +
     ...
     */
    scoreActionSignificance(action) {
        let score = 0;
     ...
     */
    extractSignals(actions) {
        const signals = new Set();
        for (const action of actions) {
            // Extract from context
            if (action.context) {
                if (action.context.signals) {
                    for (const signal of action.context.signals) {
                        signals.add(signal);
                    }
                }
     ...
     */
    generateTitle(session, keyActions) {
        if (session.summary) {
     ...
     */
    inferProblem(session, keyActions) {
        if (session.summary) {
     ...
     */
    inferSolution(session, keyActions) {
        if (keyActions.length > 0) {
     ...
     * Calculate average feedback score.
     */
    calculateFeedbackScore(actions) {
