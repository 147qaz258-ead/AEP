...
export interface ExperienceStorage {
  update(capsuleId: string, updates: { gene_id?: string }): Promise<boolean>;
}

/**
 * In-memory implementation of GeneStorage for testing and development
 */
export class InMemoryGeneStorage implements GeneStorage {
  private genes: Map<string, Gene> = new Map();

  async insert(gene: Gene): Promise<void> {
    this.genes.set(gene.id, gene);
  }

  async findById(id: string): Promise<Gene | null> {
    return this.genes.get(id) ?? null;
  }

  async findByCategory(category: string): Promise<Gene[]> {
  ...
  async findByNameAndCategory(name: string, category: string): Promise<Gene | null> {
  ...
  async update(id: string, updates: Partial<Gene>): Promise<void> {
  ...
  async existsByIdPrefix(prefix: string): Promise<boolean> {
 ...
 */
export class InMemoryExperienceStorage implements ExperienceStorage {
  private experiences: Map<string, { gene_id?: string }> = new Map();

  async update(capsuleId: string, updates: { gene_id?: string }): Promise<boolean> {
 ...
 */
async function generateGeneIdWithCounter(
  name: string,
  category: string,
  existsFn: (id: string) => Promise<boolean>,
  counter: number
...
export class AssetStore {
  private geneStorage: GeneStorage;
   ...
   */
  async createGene(request: CreateGeneRequest): Promise<Gene> {
    // Validate input
    ...
    const existing = await this.geneStorage.findByNameAndCategory(
      request.name,
      request.category
    );

    if (existing) {
      throw new DuplicateGeneError(
        `Gene '${request.name}' already exists in category '${request.category}'`
      );
    }

    // Generate unique gene_id
    const geneId = await generateGeneId(
      request.name,
      request.category,
      (id) => this.geneStorage.existsById(id)
    );
   ...
   */
  async listGenes(category?: string): Promise<Gene[]> {
    if (category) {
   ...
   */
  async updateGene(geneId: string, updates: UpdateGeneRequest): Promise<Gene> {
    ...
    const allowedFields: (keyof UpdateGeneRequest)[] = ['name', 'description'];
    const updateData: Partial<Gene> = {};
    ...
    if (updates.name && updates.name !== gene.name) {
      ...
      if (existing && existing.id !== geneId) {
        throw new DuplicateGeneError(
          `Gene '${updates.name}' already exists in category '${gene.category}'`
        );
      }
    }
   ...
   */
  async validateGeneExists(geneId: string): Promise<boolean> {
   ...
   */
  async linkCapsuleToGene(capsuleId: string, geneId: string): Promise<void> {
    // Validate gene exists
