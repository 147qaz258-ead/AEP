...
// 计算相似度
function calculateSimilarity(querySignals: string[], docSignals: string[]): number {
  if (querySignals.length === 0 || docSignals.length === 0) return 0;
...
查询信号: ${querySignals.join(', ')}`,
    ...
    case 'aep_record': {
      const { problem, solution, signals = [] } = args as {
        problem: string;
        solution: string;
        signals?: string[];
      };
