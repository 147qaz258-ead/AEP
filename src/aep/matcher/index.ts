...
export function computeMatchScore(
  ...
  for (const signal of signals) {
    ...
    const isInSignalsMatch = experience.signals_match.some(
      (s) => s.toLowerCase() === signalKey.toLowerCase() ||
 ...
 */
export function signalToKey(signal: Signal): string {
...
export class ExperienceMatcher {
  ...
  matchSync(request: MatchRequest, experiences: Experience[]): MatchResult[] {
    ...
    if (candidates.length < request.limit) {
      const textMatches = this.textSimilarityMatch(
        request.signals,
        experiences.filter(
   ...
   */
  private async exactMatch(signals: Signal[]): Promise<MatchResult[]> {
    if (!this.store) {
      return [];
    }
   ...
   */
  private async semanticMatch(signals: Signal[]): Promise<MatchResult[]> {
    if (!this.vectorStore) {
      return [];
    }
   ...
   */
  private getMatchedSignals(signals: Signal[], experience: Experience): string[] {
   ...
   */
  private getStatusFilter(request: MatchRequest): ExperienceStatus[] {
...
export class InMemoryExperienceStore implements ExperienceStore {
  ...
  private signalIndex: Map<string, Set<string>> = new Map();
